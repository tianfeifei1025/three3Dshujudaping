...
import {managementStandardList} from "@/api/api.js";
...
onMounted(() => {
  managementStandardList().then(res => {
...
<template>
  <div class="left-card" ref="cardRef">
    <m-card title="管理标准" :height="height">
      <div class="manage-stand">
        <Carousel
        ...
        >
          <CarouselItem v-for="item in standardList" :key="item.id" class="carousel-item">
            ...
            <div class="c-info">
              <div class="info1"><span class="standard-title">类型:</span><span>{{ item.name || '暂无名称' }}</span></div>
              <div class="info2">
                <span class="standard-title">标准:</span>
